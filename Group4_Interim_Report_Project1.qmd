...
model2_log <- lm(log(PINCP + 1)~AGEP+AGEP^2+SCHL+factor(SEX)+factor(RAC1P), data=pums_clean)
summary(model2_log)
...
# Put the model formula in one place
#form <- PINCP ~ AGEP + I(AGEP^2) + SCHL + factor(SEX) + 
#factor(RAC1P) + factor(POBP_binary) + NP

# Use all variables except PINCP(response) and PERNP (12 month earnings, obvious conflict)
...
##### Ridge Estimates (alpha = 0)

# Fit ridge regression path (alpha = 0 switches from lasso to ridge)
# Ridge shrinks coefficients but does NOT set them to zero
ridge <- glmnet(X, y, alpha = 0)

# Perform cross-validation to find best lambda for ridge
ridge.cv <- cv.glmnet(X, y, alpha = 0)

# Fit ridge model using lambda that minimizes CV error
ridge1 <- glmnet(X, y, lambda = ridge.cv$lambda.min, alpha = 0)

# Display ridge coefficients
# All coefficients remain non-zero (no variable selection)
ridge1$beta


# Extract coefficients from cross-validated ridge model
# Default is lambda.1se unless otherwise specified
coef(ridge.cv)
```
...
# differences
setdiff(vars_min, vars_1se)   # dropped in simpler model
setdiff(vars_1se, vars_min)   # (rare, but possible)
